...
describe('show phase reveals AI hand and crib', () => {
  ...
  it('ai.hand is preserved after scoreShow (for face-up display)', () => {
    ...
    expect(after.ai.hand.map((c) => c.id)).toEqual(state.ai.hand.map((c) => c.id));
  });
  ...
  it('crib is preserved after scoreShow (for face-up display)', () => {
    ...
    expect(after.crib.map((c) => c.id)).toEqual(state.crib.map((c) => c.id));
  });
  ...
  it('winner is set when player score reaches target', () => {
    const state = makePeggingState({
      player: {
        hand: makePeggingState().player.hand,
        discards: [],
        score: 120,
      },
    });
...
describe('awardGo winner branches', () => {
  ...
  it('allows advancement when pegging win ends the round without show scoring', () => {
    const state = makePeggingState({
      phase: 'round_over',
      winner: 'player',
      handResult: null,
    ...
    expect(canAdvanceFromRound(state)).toBe(true);
  });
});
